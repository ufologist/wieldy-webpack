...
var env = {
    // 先是标明环境的类型, 例如: dev/test/stage/prod
    // 再是该环境的具体配置
    // 统一规范属性名全小写, 单词以下划线隔开, 头尾添加两个下划线, 便于查找定位,
    // 建议不要嵌套对象, 这样便于在 JS 中替换使用
    // 例如: var apiUrl = __api_base__;
    dev: {
        // 对应 webpack 配置项目 output.publicPath
        __public_path__: '/'
    },
    prod: {
        // 如果配置 __cdn_root_path__, 则会自动拼装 __public_path__ = __cdn_root_path__ + __dir__
        ...
        // __ftp_user__: '',
        // __ftp_password__: '',
        // 配置 __ftp_root_path__, 则会自动拼装 __ftp_path__ = __ftp_root_path__ + __dir__
